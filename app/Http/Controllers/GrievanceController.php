...
class GrievanceController extends Controller
{
    ...
    public function grievance_datatable(Request $request){
        ...
        if(session()->get('user_type')==0){
            ...
            $record = tbl_grievance::leftjoin('tbl_grievence_forwored','tbl_grievence_forwored.griv_code','tbl_grivense.code')->wherenull('tbl_grievence_forwored.griv_code')->select('tbl_grivense.name','tbl_grivense.mobile_no','tbl_grivense.email','tbl_grivense.complain','tbl_grivense.code')
                ->orderby('code', 'desc')
        ...
        $q->orwhere('tbl_grivense.complain', 'like', '%' . $search . '%');
         ...
         $record = tbl_grievence_forwored::join('tbl_user','tbl_user.code','tbl_grievence_forwored.to_forword')->join('tbl_grivense','tbl_grivense.code','tbl_grievence_forwored.griv_code')->select('tbl_grivense.name','tbl_grivense.mobile_no','tbl_grivense.email','tbl_grivense.complain','tbl_grivense.code')->where('tbl_user.code',$user_cd)
        ->orderby('code', 'desc')
        ->where(function($q) use ($search) {
        $q->orwhere('tbl_grivense.name', 'like', '%' . $search . '%');
        $q->orwhere('tbl_grivense.mobile_no', 'like', '%' . $search . '%');
...
public function view_user(Request $request){
        ...
        try {
           ...
           $all_data=tbl_grievance::where('code',$request->grievance_code)->select('*')->first();
            ...
            $response = array(
                'options' => $all_data //Should be changed #32
            );
        } catch (\Exception $e) {
            $response = array(
                'exception' => true,
  ...
  public function user_list(Request $request){
  	...
  	$statusCode=200;
    	...
    	try{
    		...
    		$dist=tbl_user::where('user_type','<>',0)->pluck('name','code')->all();
    		$response = array(
  ...
  public function forwored_grievance_datatable(Request $request){
        ...
        $data = array();
        $record = tbl_grievance::leftjoin('tbl_grievence_forwored','tbl_grievence_forwored.griv_code','tbl_grivense.code')->join('tbl_user','tbl_user.code','tbl_grievence_forwored.to_forword')->wherenotnull('tbl_grievence_forwored.griv_code')->select('tbl_grivense.name as gname','tbl_grivense.mobile_no','tbl_grivense.email','tbl_grivense.complain','tbl_grivense.code','tbl_user.name')
                ->orderby('code', 'desc')
        ->where(function($q) use ($search) {
        $q->orwhere('tbl_grivense.name', 'like', '%' . $search . '%');
        $q->orwhere('tbl_grivense.mobile_no', 'like', '%' . $search . '%');
